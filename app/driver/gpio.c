...
// pin in GPIO mode
	if ((config->num_pin == 0) || (config->num_pin == 2) || (config->num_pin == 4) || (config->num_pin == 5))
	{
...
void ICACHE_FLASH_ATTR gpio_pin_intr_state_set (uint32 i,
    GPIO_INT_TYPE intr_state)
{
uint32 pin_reg;
