 ...
 * Parameters   : u8 gpio_id - which gpio to use
 *                u32 gpio_name - gpio mux name
 *                u32 gpio_func - gpio function
...
*******************************************************************************/
struct single_key_param *ICACHE_FLASH_ATTR key_init_single (u8 gpio_id, u32 gpio_name, u8 gpio_func, key_function long_press, key_function short_press)
{
...
void ICACHE_FLASH_ATTR key_init (struct keys_param *keys)
{
   ...
   u32 *p_pin;
    ...
    for (i = 0; i < keys->key_num; i++) 
    {
        ...
        p_pin = (u32 *)&GPIO->pin_0_bits;
        p_pin += keys->single_key[i]->gpio_id;
        ((gpio_pin_t *)p_pin)->pin_int = 0;
        ((gpio_pin_t *)p_pin)->pin_driver = 0;
        ((gpio_pin_t *)p_pin)->gpio_source = 0;
/*        gpio_register_set (GPIO_PIN_ADDR(keys->single_key[i]->gpio_id), 
...
*/
        ...
        //enable interrupt
        gpio_pin_intr_state_set (GPIO_ID_PIN(keys->single_key[i]->gpio_id), 2);	//GPIO_PIN_INTR_NEGEDGE);
    }
...
static void ICACHE_FLASH_ATTR key_50ms_cb (struct single_key_param *single_key)
{
    ...
    if (1 == GPIO_INPUT_GET(GPIO_ID_PIN(single_key->gpio_id))) 
    {
        ...
        gpio_pin_intr_state_set (GPIO_ID_PIN(single_key->gpio_id), 2);	//GPIO_PIN_INTR_NEGEDGE);
    ...
    {
        gpio_pin_intr_state_set(GPIO_ID_PIN(single_key->gpio_id), 1);	//GPIO_PIN_INTR_POSEDGE);
    }
}
...
static void key_intr_handler (struct keys_param *keys)
{
    ...
    for (i = 0; i < keys->key_num; i++) 
    {
        ...
        {
            //disable interrupt
            gpio_pin_intr_state_set (GPIO_ID_PIN(keys->single_key[i]->gpio_id), 0);	// GPIO_PIN_INTR_DISABLE
            ...
            if (keys->single_key[i]->key_level == 1) 
            {
                ...
                gpio_pin_intr_state_set(GPIO_ID_PIN(keys->single_key[i]->gpio_id), 1);	//GPIO_PIN_INTR_POSEDGE);
            } 
            else 
            {
