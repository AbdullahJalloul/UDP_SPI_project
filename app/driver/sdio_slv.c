...
//#include "osapi.h"
//#include "gpio.h"
//#include "driver/user_interface.h"
//#include "mem.h"
...
#define SLC_INTEREST_EVENT (SLC_ENA_INT_TX_EOF | SLC_ENA_INT_RX_EOF | SLC_ENA_INT_RX_UDF | SLC_ENA_INT_TX_DSCR_ERR)
#define TRIG_TOHOST_INT()	SLC->int_vect_bits.tohost_intvect = 1;\
...
void sdio_slave_init (void)
{
	...
	////reset orginal link
	SLC->conf0 |= (SLC_CONF0_RXLINK_RST | SLC_CONF0_TXLINK_RST);
...
//	CLEAR_PERI_REG_MASK(SLC_CONF0, SLC_RXLINK_RST|SLC_TXLINK_RST);
	...
	//set sdio mode
	SLC->rx_dscr_conf |= (SLC_RX_EOF_MODE | SLC_RX_FILL_MODE);
...
//	SET_PERI_REG_MASK(SLC_HOST_INTR_ENA, SLC_HOST_TOHOST_BIT0_INT_ENA);
	...
	while (loop-- )
	{
		...
		{
			pHead_Sended = (struct sdio_list*)os_malloc (
			        sizeof(struct sdio_list));
			p = pHead_Sended;
		}
		else
		{
...
//	SET_PERI_REG_MASK(SLC_RX_DSCR_CONF, SLC_TOKEN_NO_REPLACE);
	...
	SLC->tx_link_bits.start = 1;
//	SET_PERI_REG_MASK(SLC_TX_LINK, SLC_TXLINK_START);
	SLC->host_conf_w2 = sdio_sta.word_value;
...
static void sdio_slave_isr (void *para)
{
	...
	slc_intr_status = SLC->int_status;	// READ_PERI_REG(SLC_INT_STATUS);
...
//	WRITE_PERI_REG(SLC_INT_CLR, slc_intr_status);
	...
	//FROM HOST DONE
	if (slc_intr_status & SLC_ENA_INT_TX_EOF)
	{
		...
		{
			sdio_recv_data_callback_ptr ((uint8*)tx_que.buf_ptr,
			        tx_que.datalen);
		}
		tx_buff_handle_done ();
		TRIG_TOHOST_INT()
	...
	//TO HOST underflow
	if (slc_intr_status & SLC_ENA_INT_RX_UDF)
	{
	}

	//FROM HOST overflow
	if (slc_intr_status & SLC_ENA_INT_TX_DSCR_ERR)
	{
	}
...
static void rx_buff_read_done (void)
{
	...
	sdio_sta.word_value = SLC->host_conf_w2;	// READ_PERI_REG(SLC_HOST_CONF_W2);
	sdio_sta.elm_value.comm_cnt++;
	sdio_sta.elm_value.rd_empty = 1;
	sdio_sta.elm_value.rx_length = 0;
	sdio_sta.elm_value.intr_no &= ( ~RX_AVAILIBLE);
	SLC->host_conf_w2 = sdio_sta.word_value; //update sdio status register
...
static void tx_buff_handle_done (void)
{
	...
	SLC->tx_link_bits.start = 1; //tx buffer is ready for being written
...
static int32 rx_buff_load_done (uint16 rx_len)
{
	...
	SLC->rx_link_bits.start = 1; //rx buffer is ready for being read
...
int32 ICACHE_FLASH_ATTR sdio_load_data (const uint8* data, uint32 len)
{
	...
	while (len)
	{
		left_len = RX_BUFFER_SIZE + SDIO_TOKEN_SIZE
		        - (uint32) (pHead_Sended->tail - pHead_Sended->buffer);
		if (len < left_len)
		{
...
static void sdio_read_done_process (void)
{
	...
	else if ( (pHead_Sended != NULL)
	        && (pHead_Sended->buffer != (pHead_Sended->tail - SDIO_TOKEN_SIZE)))
	{
