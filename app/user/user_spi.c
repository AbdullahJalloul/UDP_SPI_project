...
void ICACHE_FLASH_ATTR spi_init (void)
{
	...
	GPIO->pin_13_bits.pin_int = GPIO_INT_DISABLE;	// нет прерываний
	...
	GPIO->pin_4_bits.pin_int = GPIO_INT_DISABLE;	// нет прерываний
...
static void ICACHE_FLASH_ATTR gpio5_setup (void)
{
	...
	GPIO->pin_5_bits.pin_int = GPIO_INT_NEG_EDGE;	// прерывание по спаду	
...
static void __attribute__((optimize("O2"))) spi_int_handler (void *para)
{
	...
	if (dport_value.spi1_isr)
	{
		...
		{
			read_udata ();			// перенести данные из FIFO в буфер
			g.need_transmit = 1;	// для теста запустить передачу обратно
//			spi_reg_mod ();
			// TODO: запустить передачу UDP с полученным массивом
			if (g.need_transmit)	// запуск отложенной передачи
			{
				write_udata ();				// как влияет запись на регистр чтения?
				GPIO->out_w1ts = GPIO_OUT_W1TS_PIN_13;	// REQ = 1
				// подождать 100 нс для проверки на состояние мастера
				__asm__ __volatile__ ("nop");	// 1/80 MHZ = 12.5 ns
				__asm__ __volatile__ ("nop");
...
//	ets_uart_printf ("SPI1 SPI1->slave_bits = %X\n\r", regvalue);
}

/*******************************************************************************
* Передача массива по SPI. Вызывается после приёма данных по UDP
*******************************************************************************/
void ICACHE_FLASH_ATTR spi_send_massive (u32 *buf)
{
	if (GPIO->in_bits.pin_4)	// первая проверка на высокий уровень CS
	{
		write_udata ();					// записать данные в буфер
...
//			SPI1->pin = SPI_PIN_CLK1_CS0;	// режим передачи
		...
		else	// мастер передаёт данные - конфликт №3
		{
			GPIO->out_w1tc = GPIO_OUT_W1TC_PIN_13;	// REQ = 0
			g.need_transmit = 1;	// установим флаг для запуска отложенной передачи
		}	
...
void ICACHE_FLASH_ATTR write_udata (void)
{
	SPI1->w0 = b.receive[0][0];
	SPI1->w1 = b.receive[0][1];
	SPI1->w2 = b.receive[0][2];
	SPI1->w3 = b.receive[0][3];
	SPI1->w4 = b.receive[0][4];
	SPI1->w5 = b.receive[0][5];
	SPI1->w6 = b.receive[0][6];
	SPI1->w7 = b.receive[0][7];
	SPI1->w8 = b.receive[0][8];
	SPI1->w9 = b.receive[0][9];
	SPI1->w10 = b.receive[0][10];
	SPI1->w11 = b.receive[0][11];
	SPI1->w12 = b.receive[0][12];
	SPI1->w13 = b.receive[0][13];
	SPI1->w14 = b.receive[0][14];
	SPI1->w15 = b.receive[0][15];
}

void ICACHE_FLASH_ATTR read_udata (void)
{
	b.receive[0][0] = SPI1->w0;
	b.receive[0][1] = SPI1->w1;
	b.receive[0][2] = SPI1->w2;
	b.receive[0][3] = SPI1->w3;
	b.receive[0][4] = SPI1->w4;
	b.receive[0][5] = SPI1->w5;
	b.receive[0][6] = SPI1->w6;
	b.receive[0][7] = SPI1->w7;
	b.receive[0][8] = SPI1->w8;
	b.receive[0][9] = SPI1->w9;
	b.receive[0][10] = SPI1->w10;
	b.receive[0][11] = SPI1->w11;
	b.receive[0][12] = SPI1->w12;
	b.receive[0][13] = SPI1->w13;
	b.receive[0][14] = SPI1->w14;
	b.receive[0][15] = SPI1->w15;
