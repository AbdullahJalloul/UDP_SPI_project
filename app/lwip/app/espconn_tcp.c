        ...
        espconn_msg *pcurrent_msg)
{
	...
	{/*Find the server's TCP block*/
		...
		for (find_pcb = tcp_tw_pcbs; find_pcb != NULL; find_pcb =
		        find_pcb->next)
		{
			if ( (find_pcb->remote_port == remote_port)
			        && (find_pcb->remote_ip.addr == remote_ip)
...
espconn_tcp_disconnect_successful (void *arg)
{
	...
	if (pdiscon_cb != NULL)
	{
				...
				if (espconn_reuse_disabled(pdiscon_cb))
				{
					...
					while (cpcb != NULL)
					{
						...
						if (pcb_remove)
						{
							...
							{
								LWIP_ASSERT(
								        "espconn_tcp_delete: middle cpcb != tcp_tw_pcbs",
								        cpcb != tcp_tw_pcbs);
								prev->next = cpcb->next;
							}
							else
							{
								...
								LWIP_ASSERT(
								        "espconn_tcp_delete: first cpcb == tcp_tw_pcbs",
								        tcp_tw_pcbs == cpcb);
		...
		if (espconn->proto.tcp
		        && espconn->proto.tcp->disconnect_callback != NULL)
		{
	...
	if (err == ERR_OK)
	{
		...
		if (espconn_copy_disabled(ptcp_sent))
		{
			if (ptcp_sent->pcommon.ptail->unsent == 0)
			{
				...
				ets_post (espconn_TaskPrio, SIG_ESPCONN_WRITE,
				        (uint32_t)ptcp_sent);
			}
	...
	if (precv_cb->pespconn->recv_callback != NULL)
	{
		if (err == ERR_OK && p != NULL)
		{
			...
			if (length != 0)
			{
				...
				precv_cb->pespconn->recv_callback (precv_cb->pespconn, pdata,
				        length);
...
espconn_tcp_client (struct espconn *espconn)
{
	...
	else
	{
		...
		pclient->pcommon.err = tcp_connect (pcb, &ipaddr,
		        pclient->pespconn->proto.tcp->remote_port,
			...
			while (ptime_msg != NULL)
			{
				if (ptime_msg->pespconn == pspoll_cb->preverse)
				{
					...
					{/*no data sent in server's set timeout, close.*/
						if (pspoll_cb->pcommon.recv_check
						        >= ptime_msg->pcommon.timeout)
						{
							pspoll_cb->pcommon.recv_check = 0;
							espconn_server_close (pspoll_cb, pcb, 0);
						}
