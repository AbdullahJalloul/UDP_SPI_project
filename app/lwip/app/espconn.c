...
espconn_copy_partial (struct espconn *pesp_dest, struct espconn *pesp_source)
{
	...
	if (pesp_source->type == ESPCONN_TCP)
	{
		...
		os_memcpy (pesp_dest->proto.tcp->local_ip,
		        pesp_source->proto.tcp->local_ip, 4);
	...
	else
	{
		...
		os_memcpy (pesp_dest->proto.udp->local_ip,
		        pesp_source->proto.udp->local_ip, 4);
	}
 ...
 * Returns      : none
 *******************************************************************************/
sint8 ICACHE_FLASH_ATTR espconn_sent (struct espconn *espconn, uint8 *psent,
        uint16 length)
{
		...
		case ESPCONN_TCP:
			...
			if (pnode->pcommon.write_flag)
			{
				...
				else
				{
					...
					pbuf->punsent = pbuf->payload = psent;
					//pbuf->payload;
					pbuf->unsent = length;
					pbuf->len = length;
...
sint16 ICACHE_FLASH_ATTR espconn_recv (struct espconn *espconn, void *mem,
        size_t len)
{
	espconn_msg *pnode = NULL;
	bool value = false;
...
sint8 ICACHE_FLASH_ATTR espconn_regist_time (struct espconn *espconn,
        uint32 interval, uint8 type_flag)
{
	espconn_msg *pnode = NULL;
